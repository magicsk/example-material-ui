...
const VakKontrolnyBodFilter = ({ filterCallbacks }: Props) => {
  ...
  const columns = [
    <>
      <Grid container alignItems="center" paddingY={1}>
        <TextField
          ...
          variant="standard"
          title="kod"
          onChange={(event: Event) => handleOnChange('kod', event)}
          onClick={(event) => {
    ...
    <>
      <LocalizationProvider dateAdapter={AdapterDateFns}>
        <Grid container alignItems="center" paddingY={1}>
          <DateTimePicker
            value={getFieldValue('parovane') ?? null}
            ampm={false}
            label="parovane"
            onChange={(val: Date | null) => {
              handleOnChangeSetField('parovane', saveDateTime(val), false)
            }}
            renderInput={(params) => (
...
export default VakKontrolnyBodFilter
