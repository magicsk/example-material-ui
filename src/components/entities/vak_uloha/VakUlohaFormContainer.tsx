...
const VakUlohaFormContainer: React.FC<VakUlohaFormContainerProps> = ({
  View,
}) => {
  const { id } = useParams()
  ...
  const handleDelete = (
    ...
    if (values?.vak_uloha_by_pk?.platny != null) {
      values.vak_uloha_by_pk.platny = !(values?.vak_uloha_by_pk?.platny ?? true)
    } else {
      console.error("values.vak_uloha_by_pk.platny not found")
    }
...
export default VakUlohaFormContainer
