...
const VakSmenaTypFormContainer: React.FC<VakSmenaTypFormContainerProps> = ({
  View,
}) => {
  const { id } = useParams()
  ...
  const handleDelete = (
    ...
    if (values?.vak_smena_typ_by_pk?.platny != null) {
      values.vak_smena_typ_by_pk.platny = !(values?.vak_smena_typ_by_pk?.platny ?? true)
    } else {
      console.error("values.vak_smena_typ_by_pk.platny not found")
    }
...
export default VakSmenaTypFormContainer
