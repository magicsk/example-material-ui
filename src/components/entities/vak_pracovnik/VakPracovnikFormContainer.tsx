...
import {
  useVak_Pracovnik_By_PkQuery,
  useInsert_Vak_PracovnikMutation,
...
interface VakPracovnikFormContainerProps {
  View: React.FC<IViewProps>
}
const VakPracovnikFormContainer: React.FC<VakPracovnikFormContainerProps> = ({
  View,
}) => {
  const { id } = useParams()
  const navigate = useNavigate()
  const intl = useIntl()
  const [insertedData, insertMutation] = useInsert_Vak_PracovnikMutation()
  let data
  if (id !== 'create') {
    ;[data] = useVak_Pracovnik_By_PkQuery({
      variables: { id },
    })
  }
  const handleSubmit = (
  ...
  const handleDelete = (
    ...
    if (values?.vak_pracovnik_by_pk?.platny != null) {
      values.vak_pracovnik_by_pk.platny = !(values?.vak_pracovnik_by_pk?.platny ?? true)
    } else {
      console.error("values.vak_pracovnik_by_pk.platny not found")
    }
...
export default VakPracovnikFormContainer
